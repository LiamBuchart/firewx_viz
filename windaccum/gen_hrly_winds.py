...
day_files = [file for file in all_files if f"{month:02d}{day:02d}" in file]
...
for file in day_files:
    ...
    if os.path.exists(os.path.join(save_dir, sfilename)) and os.path.exists(os.path.join(save_dir, dfilename)):
        ...
        print(f"Generating the hrly wind files for: {month:02d}-{day:02d} ...")
...
# save the files
ws_all.to_netcdf(f"{save_dir}/{sfilename}.h5")
